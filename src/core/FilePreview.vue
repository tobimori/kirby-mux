    ...
    <div class="k-file-preview-thumb-column">
      <div class="k-file-preview-thumb">
        ...
        <template v-if="image.src">
          <k-coords-input :disabled="!focusable" :value="focus" @input="setFocus($event)">
        ...
        <div v-else-if="test">test</div>

        <!-- Icon -->
        <k-icon v-else :color="$helper.color(image.color)" :type="image.icon" class="k-item-icon" />
      </div>
    </div>
    ...
    <div class="k-file-preview-details">
      <dl>
        <div v-for="detail in details" :key="detail.title">
          <dt>{{ detail.title }}</dt>
          <dd>
            <k-link v-if="detail.link" :to="detail.link" tabindex="-1" target="_blank">
              /{{ detail.text }}
            </k-link>
            <template v-else>
              {{ detail.text }}
        ...
        <div v-if="image.src" class="k-file-preview-focus-info">
          ...
          <dd>
            <k-file-focus-button v-if="focusable" ref="focus" :focus="focus" @set="setFocus" />
